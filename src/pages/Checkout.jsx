...
const CheckoutReservas = import.meta.env.DEV && import.meta.env.VITE_ORDERS_DEV === 'true'
  ? lazy(() => import('../components/CheckoutReservas.jsx')) : null;
...
function Checkout() {
  ...
  return <main className="min-h-screen bg-slate-950 text-white">
    ...
    <section className="mx-auto max-w-3xl px-6 py-24 text-center">
      <h1 className="text-3xl font-light md:text-5xl">{CheckoutReservas ? "Tu pedido" : puedeFinalizar ? "Próximamente" : "Verificando tu carrito"}</h1>
      {CheckoutReservas ? <p className="mt-5 text-slate-300">{verificandoStock ? 'Verificando disponibilidad…' : errorInventario || 'Revisa tu reserva y su estado. Las prendas reservadas dejan de aparecer como disponibles en el carrito.'}</p> : puedeFinalizar ? <div className="mt-5 text-lg text-slate-300"><p>Próximamente podrás completar tu compra aquí.<br />Esta revisión de stock no reserva prendas.</p><p className="mt-6">Mientras tanto, puedes finalizar tu compra por WhatsApp.<br />Revisaremos contigo la disponibilidad final y los datos del pedido antes de confirmarlo.</p></div> : <div role="alert" className="mt-5 text-lg text-amber-200"><p>{verificandoStock ? "Verificando disponibilidad…" : errorInventario || "El carrito contiene una selección sin stock válido. Regresa para revisarla."}</p>{!verificandoStock && lineas.filter(({ invalida }) => invalida).map((linea) => <p key={`${linea.productoId}-${linea.varianteKey}-${linea.selectedSize}`} className="mt-2 text-sm">{linea.nombre} · {linea.selectedSize || "talla por confirmar"}: {linea.mensajeStock}</p>)}</div>}
      ...
      {CheckoutReservas && <Suspense fallback={<p>Cargando reservas…</p>}>
        <CheckoutReservas lineas={lineas} puedeFinalizar={puedeFinalizar} revalidar={revalidar} />
      </Suspense>}
    </section>
  </main>;
}
